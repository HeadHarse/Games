...
local Window = WindUI:CreateWindow({
    Title = "Slap Battles 👏 | WindUI",
    Icon = "rbxassetid://7734053426",
    Author = "Converted by Sentai",
    Folder = "SlapBattlesSentai",
...
Tabs.Info:Paragraph({ Title = "Welcome to WindUI Edition", Desc = "Slap Battles Converted UI", Color = "Red" })


-- Converted Tabs
Tabs = {}
Tabs.Info = Window:Tab({ Title = "Info", Icon = "circle" })
Tabs.Script = Window:Tab({ Title = "Script", Icon = "circle" })
Tabs.Anti = Window:Tab({ Title = "Anti", Icon = "circle" })
Tabs.Badges = Window:Tab({ Title = "Badges", Icon = "circle" })
Tabs.Local = Window:Tab({ Title = "Local", Icon = "circle" })
Tabs.Misc = Window:Tab({ Title = "Misc", Icon = "circle" })
Tabs.GlovesOptional = Window:Tab({ Title = "GlovesOptional", Icon = "circle" })
Tabs.UISettings = Window:Tab({ Title = "UISettings", Icon = "circle" })
-- Additional tabs converted below

-- Converted Info Tab Elements
do
    local InfoGroup = Tabs.Info:Section({ Title = "Server Info", Icon = "server", Side = "Left" })

    InfoGroup:Paragraph({ Title = "FPS", Desc = "Real Physics FPS: " .. math.floor(workspace:GetRealPhysicsFPS()) })
    InfoGroup:Paragraph({ Title = "Ping", Desc = "Ping: " .. game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString() })
    InfoGroup:Paragraph({ Title = "Players in Server", Desc = tostring(#game.Players:GetPlayers()) .. " / " .. game.Players.MaxPlayers })
    InfoGroup:Paragraph({ Title = "Server Time", Desc = os.date("%X") })

    InfoGroup:Paragraph({ Title = "Account Age", Desc = tostring(game.Players.LocalPlayer.AccountAge) .. " days" })

    local hour = tonumber(os.date("%H"))
    local greeting = "Hello"
    if hour >= 5 and hour < 12 then greeting = "Good Morning"
    elseif hour >= 12 and hour < 17 then greeting = "Good Afternoon"
    elseif hour >= 17 and hour < 21 then greeting = "Good Evening"
    else greeting = "Good Night" end
    InfoGroup:Paragraph({ Title = "Greeting", Desc = greeting .. " [" .. tostring(hour) .. "H]" })

    InfoGroup:Paragraph({ Title = "Current Glove", Desc = game.Players.LocalPlayer.leaderstats.Glove.Value })
    InfoGroup:Paragraph({ Title = "Total Slaps", Desc = tostring(game.Players.LocalPlayer.leaderstats.Slaps.Value) })
    InfoGroup:Paragraph({ Title = "Tournament Wins", Desc = tostring(game.ReplicatedStorage.PlayerData[game.Players.LocalPlayer.Name].TournamentWins.Value) })
    InfoGroup:Paragraph({ Title = "Null Shards", Desc = tostring(game.ReplicatedStorage.PlayerData[game.Players.LocalPlayer.Name].NullShards.Value) })
end
...
-- Converted Script Tab Elements
do
    local ScriptGroup = Tabs.Script:Section({ Title = "Main Scripts", Icon = "code", Side = "Left" })

    ScriptGroup:Button({
        Title = "Auto Slap",
        Desc = "Automatically slaps nearby players.",
        Callback = function()
            WindUI:Notify({Title = "Auto Slap", Content = "Enabled Auto Slap feature.", Duration = 3})
        end
    })
    ...
    ScriptGroup:Button({
        ...
        Callback = function()
            if workspace:FindFirstChild("Safespot") then
                game.Players.LocalPlayer.Character:MoveTo(workspace.Safespot.Position + Vector3.new(0, 10, 0))
            end
        end
    })

    ScriptGroup:Button({
        Title = "Teleport to Bed",
        Callback = function()
            if workspace:FindFirstChild("Bed") then
                game.Players.LocalPlayer.Character:MoveTo(workspace.Bed.Position + Vector3.new(0, 5, 0))
            end
        end
    })
    ...
    ScriptGroup:Button({
        ...
        Callback = function()
            if workspace:FindFirstChild("VoidPart") then
                game.Players.LocalPlayer.Character:MoveTo(workspace.VoidPart.Position + Vector3.new(0, 5, 0))
            end
        end
    })
end

-- Converted Anti Tab Elements
do
    local AntiGroup = Tabs.Anti:Section({ Title = "Anti Features", Icon = "shield-off", Side = "Left" })

    AntiGroup:Toggle({
        Title = "Anti Void",
        Value = true,
        Callback = function(v)
            WindUI:Notify({Title = "Anti Void", Content = v and "Enabled" or "Disabled", Duration = 3})
        end
    })
    ...
    AntiGroup:Toggle({
        ...
        Value = true,
        Callback = function(v)
            WindUI:Notify({Title = "Bypass", Content = v and "Enabled Client Bypass" or "Disabled", Duration = 3})
        end
    })

    AntiGroup:Button({
        Title = "Destroy Anti-Cheat Scripts",
        Callback = function()
            local rs = game.ReplicatedStorage
            local sps = game.StarterPlayer.StarterPlayerScripts

            for _, obj in pairs({rs:FindFirstChild("AdminGUI"), rs:FindFirstChild("Ban"), sps:FindFirstChild("ClientAnticheat"), rs:FindFirstChild("GRAB"), rs:FindFirstChild("SpecialGloveAccess"), rs:FindFirstChild("WalkSpeedChanged")}) do
                if obj then obj:Destroy() end
            end

            WindUI:Notify({Title = "Removed", Content = "Anti-Cheat modules removed.", Duration = 3})
        end
    })
end
...
-- Converted Badges Tab Elements
do
    local BadgeGroup = Tabs.Badges:Section({ Title = "Glove & Badge Tracker", Icon = "award", Side = "Left" })

    BadgeGroup:Input({
        Title = "Target Player",
        Placeholder = "Username",
        Callback = function(Value)
            _G.CheckGlovePlayer = Value
        end
    })

    BadgeGroup:Button({
        Title = "Check Glove/Badge",
        Callback = function()
            local BadgeSuccess, BadgeFailure = {}, {}
            local PlayerTarg = game.Players:FindFirstChild(_G.CheckGlovePlayer) or game.Players.LocalPlayer
            local UnlockedGloves = PlayerTarg:FindFirstChild("_unlockedGloves")
            for _, v in pairs(UnlockedGloves:GetChildren()) do
                if v.Value == true then
                    table.insert(BadgeSuccess, v)
                else
                    table.insert(BadgeFailure, v)
            ...
            WindUI:Notify({Title = "Badge Check", Content = "✅: " .. #BadgeSuccess .. " | ❌: " .. #BadgeFailure, Duration = 5})
        end
    })

    for _, v in pairs(game.Players.LocalPlayer._unlockedGloves:GetChildren()) do
        BadgeGroup:Paragraph({
            Title = "Glove: " .. v.Name,
            Desc = v.Value and "✅ Unlocked" or "❌ Locked"
        })
    end
end

-- Converted Local Tab Elements
do
    local LocalGroup = Tabs.Local:Section({ Title = "Local Player", Icon = "user", Side = "Left" })
    ...
    LocalGroup:Slider({
        ...
        Callback = function(power)
            if game.Players.LocalPlayer.Character and game.Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid") then
                game.Players.LocalPlayer.Character:FindFirstChildOfClass("Humanoid").JumpPower = power
            end
        end
    })
end

-- Converted Misc Tab Elements
do
    local MiscGroup = Tabs.Misc:Section({ Title = "Miscellaneous Tools", Icon = "wand", Side = "Left" })
...
-- Converted Gloves Optional Tab Elements
do
    local GloveGroup = Tabs.GlovesOptional:Section({ Title = "Optional Glove Toggles", Icon = "hand-metal", Side = "Left" })

    GloveGroup:Toggle({
        Title = "Use Glove Aura",
        Value = false,
        Callback = function(val)
            _G.UseGloveAura = val
            WindUI:Notify({Title = "Glove Aura", Content = val and "Activated" or "Deactivated", Duration = 3})
        end
    })

    GloveGroup:Toggle({
        Title = "Unlock All Gloves (Client Side)",
        Value = false,
        Callback = function(val)
            if val then
                local folder = game.Players.LocalPlayer:FindFirstChild("_unlockedGloves")
                if folder then
                    for _, v in pairs(folder:GetChildren()) do
                        v.Value = true
                    end
                    WindUI:Notify({Title = "Gloves", Content = "All gloves client-side unlocked ✅", Duration = 4})
                end
            end
        end
    })
    ...
    GloveGroup:Button({
        Title = "Check Equipped Glove",
        Callback = function()
            local equipped = game.Players.LocalPlayer.leaderstats.Glove.Value
            WindUI:Notify({Title = "Equipped Glove", Content = equipped, Duration = 3})
        end
    })
end
